...
class TestExtractClaims:
    ...
    def test_returns_empty_list_on_parse_error(self):
        with patch("verdikt.utils.llm_client.complete", return_value="not valid json at all"):
            claims = extract_claims("text")
        assert claims == []

    def test_returns_empty_list_on_empty_array(self):
        with patch("verdikt.utils.llm_client.complete", return_value="[]"):
            claims = extract_claims("text")
        assert claims == []
    ...
    def test_passes_document_in_user_message(self):
        with patch("verdikt.utils.llm_client.complete", return_value='["c"]') as mock_complete:
